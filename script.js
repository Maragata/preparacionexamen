...
// Variables de estado
let currentQueue = []; // Cola de preguntas actual
let wrongQueue = [];   // Cola para la siguiente ronda (los fallos)
let currentIndex = 0;
let roundCount = 1;
...
function startGame() {
    // Inicializamos con todas las preguntas
    // Usamos [...] para crear una copia y no modificar el original
    currentQueue = [...fullQuestionBank]; 
    wrongQueue = [];
    currentIndex = 0;
    roundCount = 1;

    // Barajar las preguntas iniciales (Opcional, si quieres orden aleatorio)
...
function updateRoundDisplay() {
    ...
    // Si estamos en ronda de repaso (ronda > 1), cambiamos el color o texto si quieres
    if(roundCount > 1) {
...
function showQuestion() {
    ...
    // Verificar si terminamos la cola actual
    ...
    // Renderizar opciones (Barajarlas también para que no siempre salga la correcta en el mismo sitio)
    ...
    shuffledOptions.forEach(opt => {
        ...
        // Detectar si es correcta por el asterisco
...
function selectAnswer(e) {
    ...
    Array.from(optionsContainer.children).forEach(btn => {
        ...
        if (btn.dataset.correct === "true") {
            btn.classList.add('correct'); // Mostrar cuál era la correcta
        }
    });
    ...
    if (isCorrect) {
        ...
        // LÓGICA CLAVE: Guardar en la cola de fallos
...
function handleEndOfRound() {
    ...
    if (wrongQueue.length > 0) {
        ...
        currentQueue = [...wrongQueue]; // La cola actual ahora son solo los fallos
        wrongQueue = []; // Limpiamos la cola de fallos para la nueva ronda
        currentIndex = 0;
        roundCount++;
